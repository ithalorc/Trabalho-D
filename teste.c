...
FILE *fp;
int x = 0, cont = 0, i = 0, m = 1, co = 0, l, k, verif = 1;
char vetArquivos[2][15] = {"candidatos.000","candidatos.001"};


Candidato c[100], aux, v[100];
	...
	for(k = 0; k < i-m+1 ; k++){
		...
		printf("c[m-1] = %d %s e c[i-m] = %d %s\n",m-1,c[m-1].nome,i-m,c[i-m].nome);
		v[m-1] = c[0];
		c[0] = c[i-m];
		m++;
	}
...
//	fp = fopen("candidatos.txt","wb");
	...
	//for(k = 0; k < i; k++)
	//	fwrite(&c[0],sizeof(Candidato),1,fp);
//	fclose(fp);
	/* fim da escrita */
